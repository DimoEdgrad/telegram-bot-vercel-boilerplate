...
// ==================== پیکربندی و اجرای سرورلس با پاسخ فوری ====================
export const startVercel = async (req: VercelRequest, res: VercelResponse) => {
  try {
    // خط کلیدی زیر: فوراً به تلگرام پاسخ مثبت داده و اتصال را برای درخواست‌های بعدی آزاد می‌کند
    if (res && typeof res.status === 'function') {
      res.status(200).send('OK');
    }
    // سپس پردازش کد ربات در پس‌زمینه بدون مسدود کردن سرور انجام می‌شود
