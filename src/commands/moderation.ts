...
async function hasModerationPermission(userId: number | undefined): Promise<boolean> {
...
export const handleBan = async (ctx: Context) => {
  ...
  if (replyTo && typeof replyTo === 'object' && 'from' in replyTo && replyTo.from) {
...
export const handleWarn = async (ctx: Context, warns: any) => {
  ...
  const replyTo = (ctx.message as any)?.reply_to_message;
  if (!replyTo || typeof replyTo !== 'object' || !('from' in replyTo) || !replyTo.from) return ctx.reply('❌ روی پیام کاربر ریپلای کنید.');
  ...
  if (warns[chatId][targetId] >= 3) {
    ...
    try { await ctx.restrictChatMember(targetId, { permissions: { can_send_messages: false } }); ctx.reply(`🚫 ${replyTo.from.first_name} ۳ اخطاره و سکوت شد.`); } catch {}
  } else {
